...
// Arrotonda a 2 decimali evitando i tipici errori di floating point
// (es. 87.46999999999998 -> 87.47)
function round2(n){
  return Math.round((n + Number.EPSILON) * 100) / 100;
}

// ── ESPORTA CSV (stesso formato di portafoglio_reale_input.csv) ─────
// Colonne: isin,nome,valore_eur,pl_acquisto_eur,quantita,prezzo_eur
async function exportCSV(){
  if(!portfolio.length){ toast('Nessuna posizione da esportare'); return; }
  toast('Esportazione in corso…');

  let fallbackCount = 0;

  const rows = await Promise.all(portfolio.map(async p=>{
    let current = await getCurrentPrice(p.ticker);
    if(current == null){
      current = p.price; // fallback: prezzo di acquisto se il prezzo live non è disponibile
      fallbackCount++;
    }
    const valore = current * p.qty;
    const pl     = valore - (p.price * p.qty);
    return {
      isin: p.isin || '',
      nome: p.name || '',
      valore_eur: round2(valore),
      pl_acquisto_eur: round2(pl),
      quantita: p.qty,
      prezzo_eur: round2(current)
    };
  }));
  ...
  if(saldoContanti){
    rows.push({
      isin: 'SALDO_CONTANTI',
      nome: saldoContanti.name || 'Saldo contanti',
      valore_eur: round2(saldoContanti.value),
      pl_acquisto_eur: 0,
      quantita: 1,
      prezzo_eur: round2(saldoContanti.value)
    });
  }

  const header = 'isin,nome,valore_eur,pl_acquisto_eur,quantita,prezzo_eur';
  const body   = rows.map(r=>
    `${r.isin},${r.nome},${r.valore_eur},${r.pl_acquisto_eur},${r.quantita},${r.prezzo_eur}`
  ).join('\n');
  ...
  toast(fallbackCount
    ? `✓ Esportato (⚠ ${fallbackCount} prezzi non disponibili, usato prezzo di acquisto)`
