...
async function runBacktest(){
  ...
  if(strategy === 'ema40'){
    trades = strategyEMA40(ohlc, capital, feePct);
  } else if(strategy === 'sp2'){
...
// ── STRATEGIA EMA40 TREND FOLLOWING ──────────────────────────────────
//
// Logica weekly:
//   BUY  : close > EMA40 (≡ EMA200 daily) — titolo in trend rialzista
//   SELL : close < EMA40 — oppure trailing stop ATR×2 dal picco
//
// Warm-up: 40 barre
//
function strategyEMA40(ohlc, capital, feePct){
  const closes  = ohlc.map(x => x.close);
  const times   = ohlc.map(x => x.time);
  const ATR_MULT = 2.0;

  const e40arr = emaArr(closes, 40);
  ...
  for(let i = 40; i < ohlc.length; i++){
    const price = closes[i];
    const e40   = e40arr[i];
    const atr   = sp2ATR(ohlc, i);

    if(!inTrade){
      // BUY: close supera EMA40 dal basso
      if(price > e40){
        const cost = cash * (1 - feePct);
        shares  = cost / price;
        cash    = 0;
      ...
      if(price > peak) peak = price;
      const trailStop = peak - atr * ATR_MULT;
      const sellEMA   = price < e40;
      const sellTrail = price <= trailStop && peak > entry.price * 1.02;

      if(sellEMA || sellTrail){
        const value = shares * price * (1 - feePct);
        const pl    = value - entry.price * entry.shares;
        const plpct = pl / (entry.price * entry.shares);
        log.push({
          ...
          entryPrice: entry.price, exitPrice: price,
          pl, plpct,
          exitReason: sellEMA ? 'EMA40' : 'Trailing'
        });
        cash = value; shares = 0; inTrade = false; entry = null; peak = 0;
      }
    }
